...
public class ElectionsWinners {
    public static void main(String[] args) {
        int[] votes = {2, 3, 5, 2}; // 2
        int k = 3;
       /* int[] votes = {1, 3, 3, 1, 1};  // 0
        int k = 0;*/
   /*     int[] votes = {1, 1, 1, 1};  // 4
        int k = 1;*/
      /*  int[] votes = {3, 1, 1, 3, 1};  // 2
        int k = 2;*/

  /*      int[] votes = {5, 1, 3, 4, 1};  // 1
        int k = 0;*/
    ...
    static int solution(int[] votes, int k) {
        ...
        int winner = 0;
        boolean thereIsTie = votes[votes.length - 2] == max;
        if(!thereIsTie && k == 0) return 1;
...
/*  solução colega
    ...
    return k == 0 ? Arrays.stream(votes).filter(x -> x == max).count() == 1 ?
...
votes for each of his opponents, he will still be the winner (the votes
