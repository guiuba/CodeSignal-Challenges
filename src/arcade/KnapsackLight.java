...
public class KnapsackLight {
    ...
    static int solution(int value1, int weight1, int value2, int weight2, int maxW) {
        ...
        if(map.get(map.lastKey()) <= maxW) {
            value += map.lastKey();
            loadedWeight += map.get(map.lastKey());
        }
        if(map.get(map.firstKey()) + loadedWeight <= maxW) { //map.get(map.firstKey()) <= maxW &&
            value += map.firstKey();
        }

     //   map.forEach(map.entrySet() -> System.out.println(map.entrySet()));
